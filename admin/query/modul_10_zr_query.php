...
if (isset($_POST['add_soal'])) {
    $pindah_html    = '../soal/modul_10_zr';



    $cekNomorSoal   = $soal->CekNomorSoal($_POST['add_nomor'], 'modul_10_zr');
    switch ($cekNomorSoal) {
        case 'TIDAK ADA':
            $nama_modul = 'modul_10_zr';
            $pilihan    = '';
            ...
            if (empty($_POST['pilihan_a_' . $_POST['add_nomor'] . '']) || empty($_POST['pilihan_b_' . $_POST['add_nomor'] . '']) || empty($_POST['pilihan_c_' . $_POST['add_nomor'] . '']) || empty($_POST['pilihan_d_' . $_POST['add_nomor'] . '']) || empty($_POST['pilihan_e_' . $_POST['add_nomor'] . ''])) {
                header('location:../soal/modul_10_zr?soal_id=' . $_POST['add_soal_id'] . '&status=20');
            } else {
...
if (isset($_POST['update_soal'])) {
    ...
    $soal->UpdateSoalModul($_POST['up_soal_id'], 'modul_10_zr', $arr_kolom, $arr_data, $pindah_html, 9, 10);
}
...
if (isset($_POST['delete_soal'])) {
    $pindah_html = '../soal/modul_10_zr';
    $soal->HapusSoalModul($_POST['update_id'], $_POST['update_soal_id'], 'modul_10_zr', $pindah_html, 13, 14);
}
