                    ...
                    <?php if ($skor->num_rows > 0) : ?>
                      <?php while ($r_skor = $skor->fetch_assoc()) : ?>
                        <tr>
                          ...
                          <td>
                            <?php
                            if ($r_skor['skor_10'] >= 119) {
                              echo 'BS B:' . $r_skor['skor_10'];
                            } elseif (118 >= $r_skor['skor_10'] and $r_skor['skor_10'] >= 115) {
                              echo 'B B:' . $r_skor['skor_10'];
                            } elseif (114 >= $r_skor['skor_10'] and $r_skor['skor_10'] >= 111) {
                              echo 'C+ B:' . $r_skor['skor_10'];
                            } elseif (110 >= $r_skor['skor_10'] and $r_skor['skor_10'] >= 99) {
                              echo 'C B:' . $r_skor['skor_10'];
                            } elseif (98 >= $r_skor['skor_10'] and $r_skor['skor_10'] >= 95) {
                              echo 'C- B:' . $r_skor['skor_10'];
                            } elseif (94 >= $r_skor['skor_10'] and $r_skor['skor_10'] >= 50) {
                              echo 'K B:' . $r_skor['skor_10'];
                            } elseif (50 > $r_skor['skor_10']) {
                              echo 'KS B:' . $r_skor['skor_10'];
                            }
                            ?>
                          </td>
                          <td>
                            <a href="d_jawaban?room_id=<?= $r_skor['room_id'] ?>&jawaban_id=<?= $r_skor['id'] ?>&peserta_id=<?= $r_skor['peserta_id'] ?>&nama_peserta=<?= $r_skor['nama_peserta'] ?>" class="btn btn-primary">Detail Jawaban</a>
                          </td>
